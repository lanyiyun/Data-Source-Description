...
`Location` information was used to acquire `latitude` and `longtitude` from Google API with JSON so that locations can be visualized on a Google map of Chicago neighborhoods. 
